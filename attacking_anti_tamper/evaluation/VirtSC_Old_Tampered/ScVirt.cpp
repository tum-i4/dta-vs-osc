 ...
 */
static cl::list<std::string> TargetFunctions("virtualize", cl::desc("List of functions to virtualize"));

/**
 * Connectivity parameter for the safe-guards.
 */
static cl::opt<uint32_t> Connectivity("connectivity", cl::desc("The connectivity of safe-guards"), cl::init(0));

static cl::opt<bool>
  SensitiveChecks("senschecks", cl::desc("Functions marked sensitive can check other functions"), cl::init(false));

static cl::opt<std::string>
  DumpStatFile("dump-file", cl::desc("File path to dump pass stats in json format."), cl::init(""));
...
/**
 * General form to implement virtualiazation of an instruction in the switch statement below.
 * Cast Elem.OldInst to the actual type and dispatch to the VirtBuilder.
 */
...
/**
 * Controlflow-changing instructions such as Br, IndirectBr, ... pass more arguments
 * to the VirtBuilder.
 */
...
// modified such that all non-empty functions in the module are virtualized and a sensitive dummy function is added
std::vector<Function *> ScVirt::getFunctions(Module &M) {
  ...
  // add sensitive dummy function computing param1+param2; inline creation here to not modify ScVirt class
  {
    ...
    auto *FTy =
      FunctionType::get(
        ...
        {Int64Ty, Int64Ty},
        /*VarArg=*/false);
    auto *NewC = M.getOrInsertFunction("scvirt_dummy", FTy);
    auto *NewF = cast<Function>(NewC);
    auto *V0 = &*NewF->getArgumentList().begin();
    auto *V1 = NewF->getArgumentList().getNext(V0);
    auto *Block = BasicBlock::Create(Context, "", NewF);
    IRBuilder<> Builder{Block};

    auto *AddrA = Builder.CreateAlloca(Int64Ty);
    auto *AddrB = Builder.CreateAlloca(Int64Ty);
    Builder.CreateStore(V0, AddrA);
    Builder.CreateStore(V1, AddrB);
    auto *LoadedA = Builder.CreateLoad(AddrA);
    auto *LoadedB = Builder.CreateLoad(AddrB);
    auto *Result = Builder.CreateNSWAdd(LoadedA, LoadedB);
    Builder.CreateRet(Result);

    NewF->addFnAttr("sctampered");
    Worklist.emplace_back(NewF);
  }
...
// TODO: remove later
void ScVirt::printDataArray(IRBuilder<> &Builder, AllocaInst *DataArray, uint32_t Size) {
  auto *Int8PtrTy = Type::getInt8PtrTy(Builder.getContext());

  auto *Cst = Builder.CreateBitCast(DataArray, Int8PtrTy);
  auto *PrintFn = DataArray->getModule()->getFunction("_Z16print_data_arrayPhi");
...
void ScVirt::printDataArrayMapping(const std::map<uint64_t, Value *> &Data) {
  ...
  for (const auto &Pair : Data) {
    ...
    if (Ty->isSized()) {
      dbgs() << std::to_string(Layout->getTypeAllocSize(Ty)) << "\t" << *Pair.second << "\n";
    } else {
...
void ScVirt::printCodeMapping(const std::vector<CodeMap::InstToOpcode> &CodeMap) {
  dbgs() << "\t-- CodeMapping:\n" << "\t\tInst\n";
  auto IdxStart = 0u;
...
Function *ScVirt::createNewFunction(Function &TargetFunc, Module &M) {
  ...
  auto *CFunc = M.getOrInsertFunction("", TargetFunc.getFunctionType(), TargetFunc.getAttributes());
...
inline uint16_t ScVirt::calculateChecksum(const std::vector<CodeMap::InstToOpcode> &Insts, TFunc Operator) {
  uint16_t Value = 0u;
...
uint16_t ScVirt::calculateChecksumXor(const std::vector<CodeMap::InstToOpcode> &Insts) {
  const auto CS = calculateChecksum(Insts, [](uint16_t Acc, const uint16_t Elem) -> uint64_t {
    return Acc ^ Elem;
  });
...
void ScVirt::virtualizeInstruction(VirtualizationBuilder &VirtBuilder,
  ...
  case Instruction::Unreachable: {
    ...
    // when the compiler is certain it cannot be reached -> otherwise undefined behaviour
  ...
  case Instruction::CatchRet: {
    ...
    auto *BBSwitchCatchRet = BasicBlock::Create(Context, "sw_catch_ret", NewFunc);
    Switch->addCase(CaseValue, BBSwitchCatchRet);
    Builder.SetInsertPoint(BBSwitchCatchRet);

    VirtBuilder.handleCatchReturn(Builder, CatchRet, NewFunc, BBSwitchHead, IdxAlloca);
    break;
  }
  case Instruction::Invoke: {
    ...
    auto *LpadCodeIdx = VirtBuilder.handleInvoke(Builder, Invoke, NewFunc, BBSwitchHead, IdxAlloca);
...
void printIP(IRBuilder<> &Builder, Module &M, AllocaInst *IdxAlloca, GlobalVariable *CodeArray) {
...
Function *ScVirt::createEmulator(const std::vector<CodeMap::InstToOpcode> &Insts,
  ...
  const auto CountBytes = [this](const uint64_t Acc, const std::pair<uint64_t, Value *> &Pair) {
    return Acc + Layout->getTypeAllocSize(Pair.second->getType());
  };
  const auto NumElems = std::accumulate(Data.begin(), Data.end(), 0u, CountBytes);
  ...
  auto *DataArray = Builder.CreateLoad(Type::getInt8PtrTy(Context), DataArrayAlloc, "DataArray");
  ...
  auto *CurrentGEP = Builder.CreateInBoundsGEP(CodeArray, {ConstantZero, IdxLoad});
  auto *Current = Builder.CreateLoad(CurrentGEP);

  auto *Switch = Builder.CreateSwitch(Current, BBUnreach, static_cast<unsigned int>(Insts.size()));
  ...
  for (const auto &Elem : Insts) {
    ...
    if (Elem.OldInst) {
      // normal inst
      virtualizeInstruction(VirtBuilder, Builder, Elem, NewFunc, BBSwitchHead, Switch, CaseValue, IdxAlloca);
    } else {
      // safe guard
      auto &ElemToCheck = CSMap.at(InstsInserted);
      ++GuardsInserted;

      DEBUG(dbgs() << "---- Adding Guard for Function: " << ElemToCheck << "\n");
      const auto Name = CodeArrPrefix + ElemToCheck.str();
      DEBUG(dbgs() << "\tGlobalName: " << Name << "\n");
      auto *GVarToCheck = M.getNamedGlobal(Name);
      assert(GVarToCheck && "Should exist!");
      const auto NumOpcodes = GVarToCheck->getType()->getElementType()->getArrayNumElements();

      auto *GuardBB = BasicBlock::Create(Context, "sw_sg", NewFunc);
      Switch->addCase(CaseValue, GuardBB);
      Builder.SetInsertPoint(GuardBB);

      VirtBuilder.handleSafeGuard(Builder,
                                  GVarToCheck,
                                  XorFunc,
                                  BBUnreach,
                                  NumOpcodes,
  ...
  assert(GuardsInserted == ToCheck.size() && "Missed some guards?");
...
GlobalVariable *ScVirt::createCodeArray(const std::vector<CodeMap::InstToOpcode> &Insts,
...
AllocaInst *ScVirt::createEmptyDataArray(IRBuilder<> &Builder,
                                         const std::map<uint64_t, Value *> &Data,
                                         const uint64_t Size,
                                         Module &M) {
  ...
  Builder.CreateMemSet(Malloc, Builder.getInt8(0), SizeConstant, Int8Ty->getBitWidth() / 8);
...
Function *ScVirt::virtualizeFunction(FunctionInfo &FInfo,
  ...
  // count number of safe-guards
  const auto SafeGuards = std::accumulate(CodeMapping.begin(), CodeMapping.end(), 0u, [](uint64_t Acc, const CodeMap::InstToOpcode &Elem) {
    if(Elem.OldInst) {
      return Acc;
    } else {
      return Acc + 1;
  ...
  // number of safeguards should match number of functions to check
  assert(ToCheck.size() == SafeGuards);

  // create the new function and emulator
  auto *NewFunc = createEmulator(CodeMapping, DataMapping, ToCheck, CSMap, *Func, CodeArray, M);
...
void ScVirt::moveAllocaUp(Function &F) {
  ...
  DEBUG(dbgs() << "\t-- Moved " << std::to_string(Allocs.size()) << " allocas\n");
}

std::vector<ConnectivityMap> ScVirt::generateDAG(std::vector<Function *> &&Funcs) {
  // TODO: sanity checks for Funcs.size(), Sensitive.size() and Connectivity.getValue()
  ...
  // modified such that only functions that have modified code arrays should be checked
  static const auto IsSensitive = [](const Function *F) -> bool {
    return F->hasFnAttribute("sctampered");
  };
  ...
  // modified such that all functions end up being checkers of each sensitive function
  ActualConnectivity = Normal.size();
        ...
        if (SensitiveChecks.getValue()) {
          ...
          do {
            ...
            if (CheckedBy.Func != Sens) {
              ...
              const auto Iter = std::find(CheckedBy.ToCheck.begin(), CheckedBy.ToCheck.end(), Sens);
  ...
  while (Free.size() > 0) {
    ...
    for (auto I = 0u; I < Top->Outgoing.size(); ++I) {
      ...
      Neighbour->Incoming.erase(
        std::find(Neighbour->Incoming.begin(), Neighbour->Incoming.end(), Top)
      );
  ...
  while (!TopoSort.empty()) {
    ...
    auto Iter =
      std::find_if(Tmp.begin(), Tmp.end(), [&Top](const ScFunction &SC) -> bool { return SC.Func == Top->Func; });
...
bool ScVirt::runOnModule(Module &M) {
  ...
  // create the xor func if we have at least two functions
  if (WorkList.size() > 1) {
    XorFunc = createXORFunction(M);
    assert(XorFunc);
  }
  ...
  for(auto &SCF : WorkList) {
    ...
    CodeArrMap.emplace(Name, std::distance(inst_begin(SCF.Func), inst_end(SCF.Func)));
  }
  ...
  for (auto &SCF : WorkList) {
    ...
    // modified to tamper with code array if function is sensitive
    const auto ActualCSum = calculateChecksumXor(CodeMapping.getElements());
    const auto CSum = SCF.Func->hasFnAttribute("sctampered") ? ActualCSum ^ 1 : ActualCSum;
    CSMap.emplace(std::make_pair(SCF.Func->getName(), CSum));

    FunctionInfo FI{SCF.Func, CodeMapping, DataMapping};

    createCodeArray(CodeMapping.getElements(), CodeArrPrefix + FI.Func->getName().str(), M);
